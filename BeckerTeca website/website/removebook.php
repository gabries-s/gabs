...
<head>
    ...
    <title>BeckerTeca - Remover</title>
...
<body>
    ...
    <header>
         <h1 onclick="window.location='cpanel.php';" style="cursor: pointer; font-size: 20px"><?php echo $configurations['titulo_site']; ?> - Remover livro</h1>
    </header>
...
<form method="POST">
    ...
    <div class="tool-section">
        ...
        <p>   
		 ...
		 <label for="book_release_data">Data de lançamento: (dd-mm-aaaa)</label>
        <div>
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    // Inicializa as variáveis com os dados do formulário
    ...
    $book_release_data = isset($_POST['book_release_data']) ? $_POST['book_release_data'] : '';

    // Caso a data de lançamento não tenha sido informada, podemos usar o valor informado
    if (empty($book_release_data)) {
        $book_release_data = date('Y-m-d');  // Usando a data atual caso não tenha sido preenchida
    }
    ...
    // Query de exclusão com base nos dados do formulário
    $sql = "DELETE FROM livros WHERE titulo = ? AND nome = ? AND autor = ? AND data_lancamento = ?";

    // Preparar a consulta
    if ($stmt = $conn->prepare($sql)) {
        // Bind dos parâmetros
        $stmt->bind_param("ssss", $book_title, $book_name, $book_author, $book_release_data);

        // Executando a consulta
        if ($stmt->execute()) {
            if ($stmt->affected_rows > 0) {
                createModal("Aviso", "Livro removido com sucesso!", "OK");
            } else {
                createModal("Aviso", "Nenhum livro encontrado com os dados fornecidos.", "OK");
            }
        } else {
            createModal("Erro ao remover livro", $stmt->error, "OK");
        }
